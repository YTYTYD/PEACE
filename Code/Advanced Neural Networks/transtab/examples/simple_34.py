...
def pre(classflag):
    ...
    # build transtab classifier model
        if classflag==0 or classflag==1 or classflag==5 :
            ...
            model = transtab.build_classifier(cat_cols, num_cols, bin_cols, checkpoint='/media/tiffany/新加卷/PycharmProjects/transtab-main/ckpt/pretrained_3',num_class=2)
        ...
        if classflag==0:
            res_acc=transtab.evaluate(ypred, y_test, seed=123, metric='acc')
            res_auc=transtab.evaluate(ypred, y_test, seed=123, metric='auc')
            res_pre = transtab.evaluate(ypred, y_test, seed=123, metric='pre')
            res_rec = transtab.evaluate(ypred, y_test, seed=123, metric='rec')
            res_f1 = transtab.evaluate(ypred, y_test, seed=123, metric='f1')
            acc.append(res_acc)
            auc.append(res_auc)
            pre.append(res_pre)
            rec.append(res_rec)
            ...
            res_mse = transtab.evaluate(ypred, y_test, seed=123, metric='mse')
            res_mae = transtab.evaluate(ypred, y_test, seed=123, metric='mae')
            mae.append(res_mae)
            mse.append(res_mse)
