...
class Dataset_CSV(Dataset):
    def __init__(self, train=True, fold=4):

        # 打开文件用于二进制读取
        with open('./classify_2.pkl', 'rb') as file:
        ...
        if fold == 5:
            X_train, X_test, y_train, y_test, attn_train, attn_test = train_test_split(
                all_feature_list, all_label_list, all_attention_list, test_size=0.2, random_state=42
            )

            if train:
                self.feature_list = X_train
                self.label_list = y_train
                self.attention_list = attn_train
            else:
                self.feature_list = X_test
                self.label_list = y_test
                self.attention_list = attn_test
...
class Config:
    def __init__(self):
        self.seq_len = 12 # 每个患者最长有12条记录，即token
        self.pred_len = 12 # 每个token有93个特征
        self.d_model = 93
...
class Network(nn.Module):
    def __init__(self, num_labels):
        ...
        self.classifier = nn.Linear(self.configs.d_model, num_labels)

    def forward(self, input):
        # itransformer
        outputs = self.model(input, None, None, None)
...
if __name__ == '__main__':
    ...
    model_network = Network(num_labels=4).to(device)
    # 训练
    ...
    for epoch in range(epochs):
        ...
        for i, (features, labels, attentions) in enumerate(loader1):
            ...
            # 4 表示类别数量
            loss = F.cross_entropy(out.view(-1, 4), labels.view(-1), reduction='none')
        ...
        test_accuracy, test_auc, test_f1, test_recall, test_precision = evaluate_metrics(loader_test, model_network, device,num_classes=4)
