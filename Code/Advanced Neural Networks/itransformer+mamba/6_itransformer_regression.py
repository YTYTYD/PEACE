...
class Dataset_CSV(Dataset):
    def __init__(self, train=True, fold=5, regression=7):

        # 打开文件用于二进制读取
        with open('./regression2_' + str(regression) + '.pkl', 'rb') as file:
...
class Config:
    def __init__(self):
        self.seq_len = 33 # 每个患者最长有47条记录，即token
        self.pred_len = 33
        self.d_model = 99 # 每个token有99个特征
...
class Network(nn.Module):
    def __init__(self, num_labels):
        ...
        self.classifier = nn.Linear(self.configs.d_model, 64)
